...
#'
#' @param policy_ls list. A list containing an S3 bucket policy (as returned by get_bucket_policy)
#'
  ...
  for (i in 1:num_rules) {
    ...
    if ("Condition" %in% keys) {
      ...
      prefixes <- tryCatch(
        ...
        error = function(e) {
          warning(sprintf(
            "No file prefixes found in policy condition for %s",
            permissions[i]
          ))
          return(NULL)
        }
...
#'
#' @param s3_uri character. string specifying the S3 URI to make public
...
set_uri_public <- function(
  ...
  current_pub_files <- unique(subset(
    policy_df,
    permission %in% c(getID, listID),
    select = file
  ))
  ...
  if (!test) {
    jsonlite::write_json(
      policy_ls,
      file.path(tmp_dir, "previous_policy.json"),
      pretty = TRUE,
      auto_unbox = TRUE
    )
  ...
  if (!test) {
    ...
    jsonlite::write_json(
      policy_ls,
      file.path(tmp_dir, "current_policy.json"),
      pretty = TRUE,
      auto_unbox = TRUE
    )
    s3_instance$put_object(
